...
class CMIM(nn.Module):
    def __init__(self, opt):
        ...
        self.fc1 = nn.linear(opt.v_f_len + opt.i_f_len, 512)
        self.active_func_1 = nn.ReLU()

        self.fc2 = nn.linear(512 + opt.v_f_len + opt.i_f_len, opt.v_f_len + opt.i_f_len)
    ...
    def forward(self, v, i):
        ...
        out1 = self.fc1(vi)
        out1 = self.active_func_1(out1)
        out1 = torch.cat((out1, vi), dim=-1)

        out2 = self.fc2(out1)

        out2 = out2.view(batch_size, seq_len, -1)
        return out2

# FCN version of CMIM
class FCN_CMIM(nn.Module):
    def __init__(self, opt):
        super(FCN_CMIM, self).__init__()

        self.fc1 = nn.Linear(opt.v_f_len + opt.i_f_len,  256)
        self.active_func_1 = nn.ReLU()

        self.fc2 = nn.Linear(256 + opt.v_f_len + opt.i_f_len,  128)
        self.active_func_2 = nn.ReLU()

        self.fc3 = nn.Linear(128 + 256 + 2*opt.v_f_len + 2*opt.i_f_len,  opt.v_f_len + opt.i_f_len)
    ...
    def forward(self, v, i):
        ...
        out2 = self.fc2(out1)
        out2 = self.active_func_2(out2)
        out2 = torch.cat((out2, out1, vi), dim=-1)

        out3 = self.fc3(out2)

        out3 = out3.view(batch_size, seq_len, -1)
        return out3
...
class FCN_VIO(nn.Module):
    def __init__(self, opt):
        super(FCN_VIO, self).__init__()

        self.Encoders_net = Encoder(opt)
        self.CMIM = FCN_CMIM(opt)
